...
from config import COLOR_SEQ, COLORS, PLOTLY_TEMPLATE
...
def build_sidebar(df: pd.DataFrame) -> dict:
    """
    Construye el sidebar de filtros globales y retorna los valores seleccionados.

    Parameters
    ----------
    df : pd.DataFrame
        DataFrame completo (sin filtrar) para poblar las opciones de cada control.
...
def show_kpis(df: pd.DataFrame) -> None:
    """Renderiza las cuatro tarjetas KPI principales."""
...
# ─────────────────────────────────────────────
def plot_top_causes(df: pd.DataFrame, n: int = 10) -> None:
    """Gráfico de barras con análisis automático por IA."""
    ...
    # 3. ANÁLISIS AUTOMÁTICO (Sin botón ni expander)
    from data_processing import obtener_analisis_ia
    
    # Mostramos un mensaje sutil de carga solo la primera vez
    with st.spinner("Generando interpretación..."):
        # Llamamos a la IA directamente
        respuesta = obtener_analisis_ia(top, f"Top {n} causas")
        
        # Usamos un contenedor visual limpio para que parezca un pie de gráfico
        st.markdown(
            f"""
            <div style="background-color: #f8f9fa; border-left: 5px solid #1B4F72; padding: 10px; margin-top: -10px;">
                <p style="color: #1B4F72; font-size: 0.9rem; margin-bottom: 0;">
                    <strong>💡 Análisis Automático:</strong> {respuesta}
                </p>
            </div>
            """, 
            unsafe_allow_html=True
...
def plot_sex_distribution(df: pd.DataFrame) -> None:
    """Gráfico de barras de defunciones por sexo."""
...
def plot_age_distribution(df: pd.DataFrame) -> None:
    """Gráfico de barras de defunciones por grupo etario."""
    age = (
...
def section_resumen(df: pd.DataFrame) -> None:
    """Sección A: resumen general con top causas y distribuciones de sexo y edad."""
...
# ─────────────────────────────────────────────
def plot_territorial_analysis(df: pd.DataFrame) -> None:
    """
    Sección B: ranking territorial por región/provincia y heatmap
    de causa macro × territorio.
    """
...
# ─────────────────────────────────────────────
def plot_cause_analysis(df: pd.DataFrame, subgroup_cols: list[str]) -> None:
    """
    Sección C: análisis detallado por grupo macro de causa, incluyendo
    distribución por sexo, edad y subgrupos específicos detectados.
    ...
    subgroup_cols : list[str]
        Columnas de subgrupo detectadas por ``get_subgroup_columns()``.
    """
...
def plot_time_series(df: pd.DataFrame) -> None:
    """
    Sección D: serie temporal de defunciones con opción de desglose por
    causa macro, sexo o región, más un stacked bar anual por sexo.
    """
...
# ─────────────────────────────────────────────
def show_filtered_table(df: pd.DataFrame) -> None:
    """
    Sección E: tabla de datos filtrados con vista agregada o cruda,
    búsqueda libre y descarga en CSV.
    """
    ...
    with st.expander("Ver tabla de datos (filtrada según selección del sidebar)", expanded=False):
        ...
        if vista == "Agregada por causa y año":
            group_cols = [
                c for c in ["anio", "provincia", "CAUSA_grupo_macro", "causa_desc", "sexo_desc"]
                if c in df.columns
            ]
            show_df = (
        ...
        st.caption(
            f"**{show_df.shape[0]:,} registros** mostrados en pantalla · "
            "El CSV descargable contiene todos los registros filtrados."
        )
...
# NOTA METODOLÓGICA
# ─────────────────────────────────────────────
def show_methodology_note() -> None:
    """Renderiza el bloque de nota metodológica de la fuente de datos."""
    st.markdown(
        """
        <div class="info-box">
        <strong>📋 Nota metodológica</strong><br>
        <strong>Fuente:</strong> Dirección de Estadísticas e Información en Salud (DEIS),
        Ministerio de Salud de la Nación Argentina.<br>
        <strong>Período:</strong> 2005–2023 · <strong>Dataset:</strong> derivado analítico
        limpio y validado.<br>
        <strong>Unidad de análisis:</strong> la columna <code>CUENTA</code> representa
        cantidad de defunciones (registros agregados, no individuales).<br>
        <strong>Valores absolutos:</strong> todos los análisis se expresan en números
        absolutos. No se calculan tasas, porcentajes de incidencia ni prevalencia, dado
        que el dataset no contiene datos de población de referencia.<br>
        <strong>Advertencia:</strong> las comparaciones territoriales deben interpretarse
        con cautela; diferencias en volumen pueden reflejar diferencias de población y no
        necesariamente diferencias epidemiológicas reales.
        </div>
        """,
        unsafe_allow_html=True,
    )
